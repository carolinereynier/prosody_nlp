...
if [ $stage -le 0 ]; then
  local/swbd1_data_download.sh /group/corporapublic/switchboard/switchboard1/swb1
...
if [ $stage -le 2 ]; then
  ...
  local/swbd1_data_prep.sh /group/corporapublic/switchboard/switchboard1/swb1
  ...
  ../../wsj/s5/utils/prepare_lang.sh data/local/dict_nosp \
...
if [ $stage -le 4 ]; then
  ...
  ../../wsj/s5/utils/format_lm_sri.sh --srilm-opts "$srilm_opts" \
  ...
  if $has_fisher; then
    ../../wsj/s5/utils/build_const_arpa_lm.sh $LM data/lang_nosp data/lang_nosp_sw1_fsh_fg
  fi
fi
...
if [ $stage -le 7 ]; then
  ...
  for x in train eval2000 $maybe_rt03; do
    ...
    ../../wsj/s5/utils/fix_data_dir.sh data/$x
  done
fi
...
if [ $stage -le 8 ]; then
  ...
  ../../wsj/s5/utils/subset_data_dir.sh --first data/train 4000 data/train_dev # 5hr 6min
  n=$[`cat data/train/segments | wc -l` - 4000]
  ../../wsj/s5/utils/subset_data_dir.sh --last data/train $n data/train_nodev
  ...
  ../../wsj/s5/utils/subset_data_dir.sh --shortest data/train_nodev 100000 data/train_100kshort
  ../../wsj/s5/utils/subset_data_dir.sh data/train_100kshort 30000 data/train_30kshort

  # Take the first 100k utterances (just under half the data); we'll use
  # this for later stages of training.
  ../../wsj/s5/utils/subset_data_dir.sh --first data/train_nodev 100000 data/train_100k
  ../../wsj/s5/utils/data/remove_dup_utts.sh 200 data/train_100k data/train_100k_nodup  # 110hr

  # Finally, the full training set:
  ../../wsj/s5/utils/data/remove_dup_utts.sh 300 data/train_nodev data/train_nodup  # 286hr
fi
...
if [ $stage -le 10 ]; then
  ...
  (
    ...
    $train_cmd $graph_dir/mkgraph.log \
               ../../wsj/s5/utils/mkgraph.sh data/lang_nosp_sw1_tg exp/tri1 $graph_dir
...
if [ $stage -le 11 ]; then
  ...
  (
    ...
    $train_cmd $graph_dir/mkgraph.log \
               ../../wsj/s5/utils/mkgraph.sh data/lang_nosp_sw1_tg exp/tri2 $graph_dir
...
if [ $stage -le 12 ]; then
  ...
  (
    ...
    $train_cmd $graph_dir/mkgraph.log \
               ../../wsj/s5/utils/mkgraph.sh data/lang_nosp_sw1_tg exp/tri3 $graph_dir
    steps/decode.sh --nj 30 --cmd "$decode_cmd" --config conf/decode.config \
...
if [ $stage -le 13 ]; then
  ...
  ../../wsj/s5/utils/dict_dir_add_pronprobs.sh --max-normalize true \
  ...
  ../../wsj/s5/utils/prepare_lang.sh data/local/dict "<unk>" data/local/lang data/lang
  LM=data/local/lm/sw1.o3g.kn.gz
  srilm_opts="-subset -prune-lowprobs -unk -tolower -order 3"
  ../../wsj/s5/utils/format_lm_sri.sh --srilm-opts "$srilm_opts" \
  ...
  LM=data/local/lm/sw1_fsh.o4g.kn.gz
  if $has_fisher; then
    ../../wsj/s5/utils/build_const_arpa_lm.sh $LM data/lang data/lang_sw1_fsh_fg
  fi

  (
    graph_dir=exp/tri3/graph_sw1_tg
    $train_cmd $graph_dir/mkgraph.log \
               ../../wsj/s5/utils/mkgraph.sh data/lang_sw1_tg exp/tri3 $graph_dir
    steps/decode.sh --nj 30 --cmd "$decode_cmd" --config conf/decode.config \
...
if [ $stage -le 14 ]; then
  ...
  (
    ...
    $train_cmd $graph_dir/mkgraph.log \
               ../../wsj/s5/utils/mkgraph.sh data/lang_sw1_tg exp/tri4 $graph_dir
